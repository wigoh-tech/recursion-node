...
// const OPENAI_API_KEY = process.env.OPENAI_API_KEY || openai.apiKey;
...
//         1. Apply ONLY grid/flex/position/background CSS properties from the JSON layout
...
//     .min(10, "HTML too short")
//     .regex(/^<[a-z][\s\S]*>$/i, "Must be valid HTML")
...
//       const validated = htmlSchema.parse(cleaned);
      
//       // Verify meaningful optimization
//       if (validated.length > html.length * 0.9) {
...
//     // Find all undermost divs with IDs
//     const divs = $('div[id]').toArray()
//       .filter(div => $(div).parents('div[id]').length === 0);

//     console.log(`Found ${divs.length} components to optimize`);
...
//       console.log(`Processing component ${index + 1} (${originalDivCount} divs)...`);
...
//         this.templateFile = path.join(outputDir, 'bareminimum_section_0.html'); // Will be set dynamically
//         this.data1File = path.join(outputDir, 'bareminimum_templates_0.json'); // Will be set dynamically
//         this.data2File = path.join(outputDir, 'template_section_0_placeholders.json'); // Will be set dynamically
//         this.intermediateFile = path.join(outputDir, 'built-website.html');
//         this.outputFile = path.join(outputDir, 'index.html');
//         this.currentHTML = '';
//     }
...
//         // Replace nested placeholders like {{key.nestedKey}}
//         Object.keys(data).forEach(key => {
//             const value = data[key];
//             if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
//                 Object.keys(value).forEach(nestedKey => {
//                     const nestedPlaceholder = `{{${key}.${nestedKey}}}`;
//                     const regex = new RegExp(nestedPlaceholder.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g');
//                     result = result.replace(regex, value[nestedKey]);
//                 });
//             }
//         });

//         // Replace top-level placeholders like {{template-n}} or {{key}}
//         Object.keys(data).forEach(key => {
...
//             // Read data1.json
//             const data1Text = await fs.readFile(this.data1File, 'utf8');
            
//             // Parse JSON data
//             let data1Content;
//             try {
//                 data1Content = JSON.parse(data1Text);
//             } catch (error) {
//                 throw new Error(`JSON parsing error in ${this.data1File}: ${error.message}`);
//             }
            
//             // Replace placeholders (await the Promise)
//             const processedHTML = await this.replacePlaceholders(templateContent, data1Content);
            
//             // Save to intermediate file
//             await fs.writeFile(this.intermediateFile, processedHTML, 'utf8');
...
//             const intermediateContent = await fs.readFile(this.intermediateFile, 'utf8');
            
//             // Read data2.json
//             const data2Text = await fs.readFile(this.data2File, 'utf8');
            
//             // Parse JSON data
//             let data2Content;
//             try {
//                 data2Content = JSON.parse(data2Text);
//             } catch (error) {
//                 throw new Error(`JSON parsing error in ${this.data2File}: ${error.message}`);
//             }
            
//             // Convert data2.json format (with id field) to key-value pairs
//             const processedData2 = {};
//             if (Array.isArray(data2Content)) {
//                 data2Content.forEach(item => {
//                     if (item.id) {
//                         processedData2[item.id] = item.innerHTML || item.textContent || item;
//                     }
//                 });
//             } else if (data2Content.id) {
//                 processedData2[data2Content.id] = data2Content.innerHTML || data2Content.textContent || data2Content;
//             } else {
//                 Object.assign(processedData2, data2Content);
//             }
            
//             // Replace placeholders (await the Promise)
//             const finalHTML = await this.replacePlaceholders(intermediateContent, processedData2);
...
//     async buildSite() {
//         try {
//             // Stage 1: bareminimum_section_0.html + bareminimum_templates_0.json → built-website.html
//             const stage1Success = await this.buildStage1();
//             if (!stage1Success) {
//                 return false;
//             }
            
//             // Stage 2: built-website.html + template_section_placeholders.json → index.html
//             const stage2Success = await this.buildStage2();
//             if (!stage2Success) {
//                 return false;
//             }
            
//             console.log('✅ Final website assembled: index.html');
//             return true;
...
// // Add this new function to assemble the final website
// async function assembleFinalWebsite(sectionIndex, outputDir) {
//     console.log(`   🏗️ Step 4: Assembling final website for section ${sectionIndex}...`);
...
//         const success = await builder.buildSite();
        
//         if (!success) {
//             throw new Error('Failed to assemble final website');
//         }
        
//         // Read the final HTML
//         const finalHtml = await fs.readFile(builder.outputFile, 'utf8');
        
//         console.log(`   ✅ Final website assembled, saved to index.html`);
...
//     } catch (error) {
//         console.error('❌ Final assembly failed:', error.message);
//         throw error;
//     }
// }
...
//     // Create a master HTML file that will contain all sections
//     let masterHtml = `
// <!DOCTYPE html>
// <html lang="en">
// <head>
...
//     <title>Migrated Website</title>
//     <style>
//         body { margin: 0; padding: 0; }
//         .migrated-section { margin-bottom: 40px; border-bottom: 1px solid #eee; padding-bottom: 20px; }
//     </style>
// </head>
// <body>
// `;
...
//             // Add this section to the master HTML
//             masterHtml += `
//     <div class="migrated-section" data-section-index="${i}">
//         ${step4Result.finalHtml}
//     </div>
// `;
...
//     // Also save the built-website.html with just the first section
//     const firstSectionHtml = await fs.readFile(
//         path.join(outputDir, 'bareminimum_section_0.html'),
//         'utf8'
//     );
//     await fs.writeFile(
//         path.join(outputDir, 'built-website.html'),
//         firstSectionHtml
//     );
...
// Replace the existing assembleFinalWebsite function with this updated version
async function assembleFinalWebsite(sectionIndex, outputDir) {
    console.log(`   🏗️ Assembling final website for section ${sectionIndex}...`);
    
    try {
        // Create an in-memory builder instance
        const builder = new WebsiteBuilder(outputDir, sectionIndex);
        
        // Build the website in memory
        const result = await builder.buildWebsite();
        
        if (!result.success) {
            throw new Error('Failed to assemble website');
        }
        
        console.log(`   ✅ Final website assembled for section ${sectionIndex}`);
        
        return {
            finalHtml: result.html,
            finalFile: null, // No file created
            inMemory: true
        };
        
    } catch (error) {
        console.error(`❌ Final assembly failed for section ${sectionIndex}:`, error.message);
        throw error;
    }
}

// Update the WebsiteBuilder class to work in memory
class WebsiteBuilder {
    ...
    async loadSectionData() {
        try {
            ...
            this.templateContent = await fs.readFile(
                path.join(this.outputDir, `bareminimum_section_${this.sectionIndex}.html`), 
                'utf8'
            );
            
            // Read data1 content
            const data1Text = await fs.readFile(
                path.join(this.outputDir, `bareminimum_templates_${this.sectionIndex}.json`), 
                'utf8'
            );
            this.data1Content = JSON.parse(data1Text);
            
            // Read data2 content
            const data2Text = await fs.readFile(
                path.join(this.outputDir, `template_section_${this.sectionIndex}_placeholders.json`), 
                'utf8'
            );
            const rawData2 = JSON.parse(data2Text);
            
            // Convert data2 to key-value pairs
            this.data2Content = {};
            if (Array.isArray(rawData2)) {
                rawData2.forEach(item => {
                    if (item.id) {
                        this.data2Content[item.id] = item.innerHTML || item.textContent || item;
                    }
                });
            } else if (rawData2.id) {
                this.data2Content[rawData2.id] = rawData2.innerHTML || rawData2.textContent || rawData2;
            } else {
                Object.assign(this.data2Content, rawData2);
            }
    ...
    async buildWebsite() {
        try {
            ...
            const loaded = await this.loadSectionData();
            if (!loaded) {
                throw new Error('Failed to load section data');
            }
            
            // Stage 1: Process with data1
            const intermediateHTML = await this.replacePlaceholders(this.templateContent, this.data1Content);
            
            // Stage 2: Process with data2
            const finalHTML = await this.replacePlaceholders(intermediateHTML, this.data2Content);
...
// Update the processAllSections function to use in-memory assembly
async function processAllSections(rawHtmlContent, computedStyles, outputDir) {
    console.log('🚀 Starting enhanced section-by-section migration process...\n');
    ...
    for (let i = 0; i < sectionFiles.length; i++) {
        ...
        try {
            ...
            // Step 4: Assemble final website in memory
            const step4Result = await assembleFinalWebsite(
                i, outputDir
            );
...
// Then update the API endpoints to use in-memory building
app.post('/api/rebuild-section/:index', async (req, res) => {
    try {
...
// Update the frontend to handle the in-memory rebuild
// Add this to your HTML interface
const rebuildInterface = `
<div style="margin: 20px 0; padding: 20px; background: #f8f9fa; border-radius: 5px;">
    <h3>🔧 Section Rebuild Tools</h3>
    <div id="sectionControls">
        <p>Select a section to rebuild:</p>
        <select id="sectionSelect" style="padding: 8px; margin-right: 10px;"></select>
        <button onclick="rebuildSelectedSection()" class="button secondary">Rebuild Selected Section</button>
    </div>
    <div id="sectionStatus" style="margin-top: 10px;"></div>
    
    <div id="previewContainer" style="margin-top: 20px; display: none;">
        <h4>Section Preview</h4>
        <iframe id="sectionPreview" style="width: 100%; height: 500px; border: 1px solid #ddd;"></iframe>
    </div>
</div>
`;

// Add these functions to your frontend JavaScript
async function loadSections() {
    try {
        ...
        if (data.sections && data.sections.length > 0) {
            data.sections.forEach(index => {
                const option = document.createElement('option');
                option.value = index;
                option.textContent = `Section ${index}`;
...
async function rebuildSelectedSection() {
    ...
    const previewFrame = document.getElementById('sectionPreview');
    
    statusDiv.innerHTML = '<div class="status loading">🔄 Rebuilding section ' + index + '...</div>';
    previewContainer.style.display = 'none';
    
    try {
        const response = await fetch(`/api/rebuild-section/${index}`, {
            method: 'POST'
        });
        const result = await response.json();
        
        if (result.success) {
            statusDiv.innerHTML = '<div class="status success">✅ Successfully rebuilt section ' + index + '</div>';
            
            // Display the rebuilt section directly in the iframe
            ...
            statusDiv.innerHTML = '<div class="status error">❌ Failed to rebuild section ' + index + ': ' + (result.message || 'Unknown error') + '</div>';
        }
    } catch (error) {
...
// Add this to your main HTML template where you want the rebuild interface to appear
// For example, after the main controls:
// ${rebuildInterface}

// Call loadSections when the page loads
