...
// BGLAYERS and TOPMOST optimization prompts remain the same as in your original code
...
async function optimizeWithAI(prompt) {
  try {
...
async function optimizeBgLayersWithAI(html, id) {
  const prompt = `${BGLAYERS_OPTIMIZATION_PROMPT}\n${html}`;
  return optimizeWithAI(prompt);
}

async function optimizeTopMostWithAI(html, id) {
  const prompt = `${TOPMOST_OPTIMIZATION_PROMPT}\n${html}`;
  return optimizeWithAI(prompt);
}
...
class ThreadSafeHtmlProcessor {
  ...
  async replaceElement(element, replacement) {
...
// Worker Functions
// async function bgLayersWorker(html, id, outputDir) {
//   const release = await fileMutex.acquire();
//   try {
//     const optimized = await optimizeBgLayersWithAI(html, id);
//     const bgKey = `bg-${Date.now().toString().slice(-4)}`;
//     return { 
//       success: true, 
//       id, 
//       html: optimized,
...
//     };
//   } catch (error) {
//     console.error(`BgLayers worker error (${id}):`, error);
...
async function bgLayersWorker(html, id, outputDir) {
  ...
  try {
    const optimized = await optimizeBgLayersWithAI(html, id);
    const bgKey = `bg-${Date.now().toString().slice(-4)}`;
    const placeholder = `{{${bgKey}}`;
    
    // Save the optimized HTML with its placeholder
    await safeWriteFile(
      path.join(outputDir, `bg_${id}_${bgKey}.html`),
      optimized
    );
...
async function topMostWorker(html, id, index, outputDir) {
  const release = await fileMutex.acquire();
  try {
    const optimized = await optimizeTopMostWithAI(html, id);
    const templateKey = `template-${String(2000 + index).padStart(4, '0')}`;
    return { 
      success: true, 
      id, 
      html: optimized,
    ...
    console.error(`TopMost worker error (${id}):`, error);
    return { success: false, id, error: error.message, index };
  } finally {
    release();
  }
}
...
async function generateBareMinimumHtml(sectionIndex, widgetsHtmlInput, outputDir) {
  console.log(`\n🚀 Starting thread-safe processing for section ${sectionIndex}`);
  
  const widgetsHtmlPath = path.join(outputDir, `widgets_extracted_${sectionIndex}.html`);
  const widgetsHtml = await safeReadFile(widgetsHtmlPath);
  const htmlProcessor = new ThreadSafeHtmlProcessor(widgetsHtml);

  // Process bgLayers
  const bgLayerDivs = [];
  htmlProcessor.$('div[id^="bgLayers"]').each((index, element) => {
    bgLayerDivs.push({
      id: htmlProcessor.$(element).attr('id'),
      element: element,
      html: htmlProcessor.$.html(element)
    });
  });

  const bgTemplates = {};
  const maxWorkers = Math.min(4, bgLayerDivs.length);
  
  for (let i = 0; i < bgLayerDivs.length; i += maxWorkers) {
    const batchResults = await Promise.all(
      bgLayerDivs.slice(i, i + maxWorkers).map(divData => 
        new Promise(resolve => {
          const worker = new Worker(__filename, {
            workerData: { type: 'bgLayers', ...divData, outputDir }
          });
          worker.on('message', async (result) => {
            if (result.success) {
              bgTemplates[result.templateKey] = result.html;
              await htmlProcessor.replaceElement(divData.element, result.placeholder);
            }
            worker.terminate();
            resolve();
          });
  ...
  await safeWriteFile(
    path.join(outputDir, `bg_${sectionIndex}.json`),
    JSON.stringify(bgTemplates, null, 2)
  );

  // Process top-most divs
  const topMostProcessor = new ThreadSafeHtmlProcessor(await htmlProcessor.getHtml());
  const topMostDivs = [];
  topMostProcessor.$('div[id]').each((index, element) => {
    const $element = topMostProcessor.$(element);
    if (!$element.attr('id').startsWith('bgLayers') && $element.parents('div[id]').length === 0) {
      topMostDivs.push({
        id: $element.attr('id'),
        element: element,
        html: topMostProcessor.$.html(element)
      });
    }
  });
  ...
  for (let i = 0; i < topMostDivs.length; i += maxWorkers) {
    const batchResults = await Promise.all(
      topMostDivs.slice(i, i + maxWorkers).map(divData => {
        processedCount++;
        return new Promise(resolve => {
          ...
          worker.on('message', async (result) => {
            if (result.success) {
              ...
              await topMostProcessor.replaceElement(divData.element, result.placeholder);
            }
            worker.terminate();
            resolve();
          });
  ...
  const finalHtml = await topMostProcessor.getHtml();
  await safeWriteFile(
    path.join(outputDir, `bareminimum_section_${sectionIndex}.html`),
    finalHtml
  );
  await safeWriteFile(
    path.join(outputDir, `bareminimum_${sectionIndex}.json`),
    JSON.stringify(componentTemplates, null, 2)
  );

  return {
    bareHtml: finalHtml,
    bareMinimumFile: `bareminimum_section_${sectionIndex}.html`,
    bgJsonFile: `bg_${sectionIndex}.json`,
    componentsJsonFile: `bareminimum_${sectionIndex}.json`,
    stats: {
      originalSize: widgetsHtml.length,
      finalSize: finalHtml.length,
      sizeReduction: Math.round((1 - (finalHtml.length / widgetsHtml.length)) * 100) + '%',
...
if (!isMainThread) {
  if (workerData.type === 'bgLayers') {
    bgLayersWorker(workerData.html, workerData.id, workerData.outputDir)
      .then(result => parentPort.postMessage(result));
  } else if (workerData.type === 'topMost') {
    topMostWorker(workerData.html, workerData.id, workerData.index, workerData.outputDir)
      .then(result => parentPort.postMessage(result));
  }
}

async function processSectionsWithWorkers(sectionFiles, outputDir) {
  const processedSections = [];
  let successfulSections = 0;

  for (const section of sectionFiles) {
    try {
      const sectionHtml = await safeReadFile(path.join(outputDir, section.htmlFile));
      const computedData = JSON.parse(
        await safeReadFile(path.join(outputDir, section.computedFile))
      );
      ...
      const step2Result = await extractWidgetsFromHtml(
        step1Result.styledHtml, 
        section.index, 
        outputDir
      );
      ...
      console.error(`Error processing section ${section.index}:`, error);
      processedSections.push({
        index: section.index,
        error: error.message,
        failed: true
  ...
  return {
    ...
    summary: {
      ...
      successfulSections,
      failedSections: sectionFiles.length - successfulSections
    }
  };
}
...
// All your Express routes remain exactly the same
app.post('/api/migrate', async (req, res) => {
    try {
        ...
        // Process sections with thread-safe implementation
        const sectionFiles = await extractAndSaveSections(rawHtmlContent, computedStyles, outputDir);
        const results = await processSectionsWithWorkers(sectionFiles, outputDir);
...
module.exports = {
    ...
    topMostWorker,
    optimizeWithAI,
    extractWidgetsFromHtml,
    extractAndSaveSections,
    EnhancedHtmlStyleProcessor,
    WebsiteBuilder,
    assembleFinalWebsite,
    processAllSections,
