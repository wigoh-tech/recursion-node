...
async function processWidgetPlaceholders(sectionIndex, sectionHtml, computedData, outputDir, globalTemplates) {
...
async function applyInlineLayoutStyles(sectionIndex, templateHtml, computedData, outputDir) {
...
`;
  ...
  if (!success) {
    ...
    try {
      const result = await applyStylesWithDualHTML(sectionIndex, templateHtml, computedData, outputDir);
      return result;
    } catch (dualError) {
...
async function applyStylesWithDualHTML(sectionIndex, templateHtml, computedData, outputDir) {
 ...
 const sectionFiles = await extractAndSaveSections(rawHtmlContent, computedStyles, outputDir);
    ...
    for (let i = 0; i < sectionFiles.length; i++) {
      ...
      await fs.readFile(path.join(outputDir, `sections/section_${i}_computed.json`), 'utf8')
            ...
            // Step 1: Process widget placeholders (using full computed data)
            const step1Result = await processWidgetPlaceholders(
                i, sectionHtml, computedData, outputDir, globalTemplates
            );
            
            // Step 2: Apply inline layout styles (using full computed data)
            const step2Result = await applyInlineLayoutStyles(
                i, step1Result.processedHtml, computedData, outputDir
            );
            ...
            processedSections.push({
                index: i,
                sectionKey: computedResults[i]?.sectionKey || `section_${i}`,
                files: {
                    original: `section_${i}.html`,
                    computed: `computed/section_${i}_computed.json`,
...
app.get('/api/files', async (req, res) => {
  try {
    ...
    const fileCategories = {
      ...
      templates: files.filter(f => f.startsWith('template_') || f === 'template.json'),
...
<body>
    <div class="container">
        ...
        <div class="step">Step 2: Save computed styles by section</div>
        <div class="step">Step 3: Process widget placeholders</div>
        <div class="step">Step 4: Apply inline styles</div>
        <div class="step">Step 5: Generate optimized HTML</div>
...
app.listen(PORT, () => {
  ...
  console.log(`   2. Save computed styles by section`);
  console.log(`   3. Process widget placeholders`);
  console.log(`   4. Apply inline styles`);
  console.log(`   5. Generate optimized HTML`);
});
